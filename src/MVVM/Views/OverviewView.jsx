      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <section className="bg-white/70 backdrop-blur-xl p-6 rounded-3xl border border-white/60 shadow-xl shadow-slate-200/50">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              className="text-indigo-600 hover:text-indigo-700 text-xs font-bold flex items-center gap-1 group"
          ...
          <div className="flex justify-between items-end gap-4 my-6">
            <div>
              <strong className="text-4xl font-black text-slate-900 tracking-tight">
              ...
              {bars.map((height, index) => (
                <div key={index} className="flex flex-col items-center gap-1.5">
                  <div className="w-3.5 bg-slate-100 h-full rounded-full flex items-end overflow-hidden p-0.5">
                    <span
                      className="w-full bg-gradient-to-t from-indigo-600 to-violet-500 rounded-full transition-all duration-500"
                      style={{ height: `${height}%` }}
                    />
                  </div>
          ...
          <div className="flex gap-4 text-xs font-medium text-slate-500 border-t border-slate-100/80 pt-3">
            <span>
              Present{" "}
              <b className="text-slate-800">{stats.totalPresent ?? 0}</b>
            </span>
            <span>
              Absent <b className="text-slate-800">{stats.totalAbsent ?? 0}</b>
            </span>
            <span>
              Late <b className="text-slate-800">{stats.totalLate ?? 0}</b>
            </span>
          </div>
        </section>
        ...
        <section className="bg-white/70 backdrop-blur-xl p-6 rounded-3xl border border-white/60 shadow-xl shadow-slate-200/50">
          ...
          <div className="space-y-2.5">
            {students
              ...
              .map((student, idx) => (
                <div
                  key={student.id || `attn-student-${idx}`}
                  className="flex items-center justify-between p-3 rounded-2xl bg-slate-50/80 border border-slate-100/80 hover:bg-slate-100/50 transition-all"
                >
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <b className="text-xs font-bold text-slate-800 block">
                        {student.name}
                      </b>
