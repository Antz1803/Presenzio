    ...
    <div className="student-portal-shell">
      ...
      <main className="student-portal-main student-access-main">
        <section className="student-access-card">
          ...
          <p>
            Use the key provided by your teacher, then enter your student ID to
          ...
          <form className="student-access-form" onSubmit={accessAssessment}>
            <label>
              Assessment Key ID
              <input
                name="assessmentAccessKey"
                autoFocus
                required
                value={accessKey}
                placeholder="e.g. ASM-4A91F2C8D0"
                onChange={(event) =>
                  setAccessKey(event.target.value.toUpperCase())
                }
              />
            </label>
            <label>
              ...
              <div className="p-5">
                ...
                <ul className="space-y-2.5 text-sm font-medium text-slate-700">
                  ...
                  <li className="flex items-center gap-2.5">
                    ...
                    <span>
                      No using the Back button or minimizing the browser
                    </span>
                  </li>
                  <li className="flex items-center gap-2.5">
                    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-red-100 text-xs text-red-600">
                ...
                <div className="mt-5 rounded-lg border border-red-200 bg-red-50 p-3.5 text-xs text-red-800">
                  <p className="font-semibold leading-relaxed">
                    ⚠️ Violations are automatically detected and reported to
                    your teacher. Your exam will be immediately submitted.
                  </p>
                </div>
              </div>
            </div>
            {message.text && (
              <p
                className={`record-save-message ${message.status}`}
                role="alert"
              >
                {message.text}
              </p>
            ...
            <button className="primary-button" disabled={submitting}>
              {submitting ? "Opening assessment…" : "Open assessment"}
            </button>
          </form>
        </section>
      </main>
