import { monthBlocks, monthDataOffset, monthDataRows } from "./constants";
...
function groupSessionsByDate(attendanceSessions) {
  ...
  [...attendanceSessions]
    ...
    .forEach((session) => {
      ...
      const time =
        String(session.sessionTime || "PM").toUpperCase() === "AM"
          ? "AM"
          : "PM";
      entry[time] = session;
...
function groupSessionsByMonth(attendanceSessions) {
  ...
  return [...monthGroups.entries()].sort(([first], [second]) =>
...
function writeMonthHeader(patches, section, students, blockStart, monthKey) {
  ...
  patchCell(
    ...
    headerRow + 2,
    7,
    section?.subject_title || section?.subject_code || "",
  );
...
function writeMonthRows(patches, entries, dataStart, students) {
  ...
  entries.slice(0, monthDataRows).forEach((entry, index) => {
    ...
    patchLiteral(
      ...
      row,
      4,
      countPresent(entry.AM, "M", students),
    );
    patchLiteral(
      patches,
      "Month",
      row,
      5,
      countPresent(entry.AM, "F", students),
    );
    patchLiteral(
      patches,
      "Month",
      row,
      6,
      countPresent(entry.PM, "M", students),
    );
    patchLiteral(
      patches,
      "Month",
      row,
      7,
      countPresent(entry.PM, "F", students),
    );
  });
}
