...
export function setMetadata(patches, section) {
  ...
  const settings = [
    ...
    ["F3", metadata.days],
    ["B4", metadata.edp],
    ["F4", metadata.sectionNo],
    ["B5", metadata.code],
    ["B6", metadata.teacher],
    ["B7", metadata.title],
  ];
  settings.forEach(([address, value]) =>
    patchAddress(patches, "Settings", address, value),
  );
  ...
  summary.forEach(([address, value]) =>
    patchAddress(patches, "Summary", address, value),
  );
  ...
  Object.values(periodSheets).forEach((sheetName) => {
    ...
    periodMetadata.forEach(([address, value]) =>
      patchAddress(patches, sheetName, address, value),
    );
  });
}
