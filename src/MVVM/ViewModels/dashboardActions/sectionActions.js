...
export function useSectionActions(context) {
  const {
    accountId, accountScoped, currentSectionId, loadLiveData, clearLiveData,
    queueOfflineChange, setSection, setSections, helpers,
  } = context;
  const { browserIsOffline } = helpers;
  const updateSection = useCallback(async (sectionId, changes) => {
    ...
    if (browserIsOffline() || !supabase) {
      await queueOfflineChange("update-section", { sectionId, changes: sectionChanges });
      setSections((current) => current.map((item) => item.id === sectionId ? { ...item, ...sectionChanges } : item));
      setSection((current) => current?.id === sectionId ? { ...current, ...sectionChanges } : current);
      return;
    }
    const { error } = await supabase.from("sections").update(sectionChanges).eq("id", sectionId);
  ...
  const deleteSection = useCallback(async (sectionId) => {
    ...
    const { data: enrollments, error: enrollmentError } = await supabase.from("enrollments")
      .select("student_id").eq("section_id", sectionId);
    ...
    if (studentIds.length) {
      const { data: remaining } = await supabase.from("enrollments")
        .select("student_id").in("student_id", studentIds);
      const remainingIds = new Set((remaining ?? []).map((item) => item.student_id));
      const orphaned = studentIds.filter((id) => !remainingIds.has(id));
      if (orphaned.length) await supabase.from("students").delete().in("id", orphaned);
    }
    await loadLiveData();
