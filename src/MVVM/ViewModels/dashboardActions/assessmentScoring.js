  ...
  return questions.map((question) => {
    ...
    const multiplier = multipleChoice
      ...
      : correct
        ? 1
        : partial
          ? (Number.isFinite(near) ? Math.max(0, Math.min(100, near)) : 50) /
            100
          : (Number.isFinite(incorrect)
              ? Math.max(0, Math.min(100, incorrect))
              : 0) / 100;
    return {
      question_id: question.id,
      answer,
      is_correct: multipleChoice || expected ? correct : null,
      points_earned: Number(question.points || 0) * multiplier,
    };
  });
}
