...
export function useAssessmentUpdateActions(context) {
  const { currentSectionId, gradingPeriods, assessmentDefinitions, loadLiveData, queueOfflineChange, setAssessmentDefinitions, setAssessmentScores, helpers } = context;
  const { browserIsOffline, createLocalId, serializeAssessmentDate } = helpers;
  const updateAssessment = async ({ assessmentId, itemNo, title, category, period, instructions, timeLimitMinutes, availableFrom, availableUntil, questions, replaceAssessmentId }) => {
      if (!currentSectionId)
