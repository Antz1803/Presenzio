...
import { countOfflineMutations, readOfflineSnapshot } from "../../lib/offlineStore";
...
export function useDashboardOfflineData(context) {
  const { setSection, setSections, setStudents, setGradeRows, setGradingPeriods, setAssessmentScores, setAssessmentDefinitions, setStudentGroups, setAssessmentAttempts, setAssessmentAttemptGrants, setAssessmentViolations, setAttendanceSessions, setSessions, setStats, setConnectionStatus, setConnectionMessage, setPendingSyncCount, loadRequestIdRef } = context;
  ...
  const loadOfflineData = useCallback(async (preferredSectionId, requestId) => {
    ...
    if (requestId == null || loadRequestIdRef.current === requestId) {
      ...
      setConnectionMessage(
        snapshot ? "Offline Ã‚· saved locally" : "Offline Ã‚· no cached class data",
      );
