...
import { formatDate, formatDay, formatShortDate, resolvePeriodCodeForSession, isNetworkError, toGradeRows, callLanApi } from "./dashboardUtils";
...
export function useDashboardLiveLoader(context) {
  const { accountId, accountScoped, currentSectionId, section, students, gradingPeriods, assessmentScores, assessmentDefinitions, studentGroups, loadOfflineData, clearLiveData, loadRequestIdRef, setSection, setSections, setStudents, setGradeRows, setGradingPeriods, setAssessmentScores, setAssessmentAttempts, setAssessmentAttemptGrants, setAssessmentViolations, setAssessmentDefinitions, setStudentGroups, setAttendanceSessions, setSessions, setStats, setConnectionStatus, setConnectionMessage } = context;
  const loadLiveData = useCallback(async (preferredSectionId) => {
    ...
    try {
      ...
      const { enrollments, allStudentsData, periods, periodGrades, combinedAssessmentScoreData, liveStudentGroups, liveAssessmentDefinitions, attemptRows, grantRows, violationRows, classSessions } = await loadDashboardRecords({ sectionData, sectionList });

      const { sessionsData, liveRoster } = buildRoster({ classSessions, periodGrades, periods, enrollments });

      const { liveStats, liveAttendanceSessions, liveSessions } = buildLiveStats({ allStudentsData, sessionsData, liveRoster, periods });
