...
export function useDashboardActions(context) {
  ...
  gradingPeriods, assessmentScores, assessmentDefinitions, studentGroups,   
...
const loadTransferPreview = useCallback(
  async ({ enrollmentId, fromSectionId, toSectionId }) => {
    if (!supabase) throw new Error("Attendance transfer requires a live database connection.");

    const { count: oldRecordCount, error: oldCountError } = await supabase
      .from("attendance_records")
      .select("id, class_sessions!inner(section_id)", { count: "exact", head: true })
      .eq("enrollment_id", enrollmentId)
      .eq("class_sessions.section_id", fromSectionId);
    if (oldCountError) throw oldCountError;

    const { data: targetSessions, error: targetSessionsError } = await supabase
      .from("class_sessions")
 ...
 * teacher's own review of its session dates — not by copying the old
 ...
 * attendance records in the previous class is deleted — the old class's
...
const transferStudent = useCallback(
  async ({ enrollmentId, studentId, fromSectionId, toSectionId, attendanceEntries = [] }) => {
    ...
    if (existing) throw new Error("This student is already enrolled in that class.");
    ...
    const markedEntries = attendanceEntries.filter((entry) => entry.sessionId && entry.status);
    ...
    // entirely once the new class's attendance has been recorded above —
  ...
  /**
   * Save a new labeled student grouping ("box") for this section. Scoring
   * (category/period/item) is chosen separately later in Group Activities,
   * so nothing here is tied to a grading period or date.
   */
const saveStudentGroup = useCallback(
  async ({ sectionId, label, groupCount, assignments, category, period, itemNo }) => {
    ...
    if (browserIsOffline() || !supabase) {
      await queueOfflineChange("delete-student-group", {
        ...
        clearScores: hasScoringSlot
          ? { category: target.category, periodCode: target.period, itemNo: target.itemNo }
          : null,
      });
